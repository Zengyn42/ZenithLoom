...
任何 LLM 都能写笔记（无论有没有 MCP），整理和索引由专职 Agent（Knowledge_Curator）统一处理。

## Design Principle

**写入门槛为零，整理交给专人。**

- 生产者（technical_architect / administrative_officer / 任何 LLM）不需要理解图谱结构、标签体系、embedding 机制
- 消费者（Knowledge_Curator + PrismRagMCP）负责所有知识工程：分类、打标签、embedding、图整理
...
生产者（任何 LLM，有没有 MCP 都行）
    ...
    └── 无 MCP：用 Write 工具 → ~/Foundation/Vault/inbox/*.md（纯文件投递）
              ...
              ~/Foundation/Vault/inbox/          ← 未分类投递箱
                    │
                    ▼
              消费者（Knowledge_Curator，有 PrismRagMCP）
                    │
                    ├── 定时扫描 inbox/ 中的新 .md
                    ├── 理解内容 → 打标签 → 选择分类目录
...
**适用场景**：Knowledge_Curator（知识管理员）、未来升级后的 technical_architect/administrative_officer
...
**适用场景**：当前的 technical_architect、administrative_officer（无 MCP 权限）

### Consumer: Knowledge_Curator's Inbox Processing

Knowledge_Curator 通过 heartbeat 或手动触发，定期处理 inbox：
...
|---|---|---|---|
| **technical_architect** | 无 PrismRagMCP | Write → inbox/ | 生产者：记录设计决策、技术经验 |
| **administrative_officer** | 无 PrismRagMCP | Write → inbox/ | 生产者：记录管理流程、运营笔记 |
| **Knowledge_Curator** | 有 PrismRagMCP | 直接 write_note() | 消费者 + 生产者：整理 inbox、管理知识库 |
...
- **关注点分离**：technical_architect 是技术架构师，不应该关心知识库的分类体系和标签一致性
- **质量控制**：Knowledge_Curator 作为知识管理员，可以保证标签不混乱、分类一致、embedding 质量
- **容错**：即使 MCP 服务挂了，technical_architect/administrative_officer 仍然可以通过 inbox 产出知识，不丢失

### Why inbox + consumer, not direct write + async index?

- **LLM 不知道 Vault 的目录结构**：如果让 technical_architect 直接写 `Architecture/xxx.md`，它需要先了解整个 Vault 的分类体系。这不是它的职责。
- **标签一致性**：如果每个 LLM 自己打标签，标签会碎片化（一个叫 `#arch`，另一个叫 `#architecture`）。集中管理更可控。
- **渐进式升级**：先 inbox 模式跑通，以后给 technical_architect/administrative_officer 加 MCP 是无缝的（多一个 write 路径，inbox 仍然可用）。

### Why move to processed/ instead of delete?

- **溯源**：可以追溯原始内容和 Knowledge_Curator 整理后的版本差异
- **防误删**：Knowledge_Curator 整理错了可以从 processed/ 恢复
- **审计**：可以定期检查 Knowledge_Curator 的整理质量
...
- `blueprints/functional_graphs/knowledge_shelf/` — Knowledge_Curator 的知识库子图
- `blueprints/role_agents/knowledge_curator/` — Knowledge_Curator 的角色定义
